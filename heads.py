...
def build_classification_head(model, device, data_set, dataset_train):
    if data_set not in ["IMNET", "CIFAR"]:
        raise ValueError("data_set must be one of [IMNET, CIFAR]")

    template = imagenet_template if data_set == "IMNET" else cifar100_template
    classnames = imagenet_classnames if data_set == "IMNET" else dataset_train.classes
